...
def random_masking_bpps(bpps, mask_ratio=0.5):
    """
    对输入bpps进行随机遮蔽。
    参数:
    - bpps: 输入bpps, 形状为 (B, C, H, W)
    - mask_ratio: 要遮蔽的像素比例
    
    返回:
    - masked_bpps: 被遮蔽的bpps
    - mask: 遮蔽矩阵
    """
    B, C, H, W = bpps.shape
    num_masking = int(mask_ratio/ 2 * H * W)  # 计算每张bpps要遮蔽的像素总数
    
    # 生成mask
    mask = torch.zeros((B, H*W), dtype=torch.bool)
    for i in range(B):
        idx = torch.randperm(H*W)[:num_masking]  # 随机选择要遮蔽的像素索引
        mask[i, idx] = True
        mask[i, H*W-idx-1] = True  # 对称遮蔽
    mask = mask.view(B, 1, H, W).expand(-1, C, -1, -1)  # 调整形状并复制到每个通道
    # 应用mask
    masked_bpps = bpps.clone()
    masked_bpps[mask] = 0  # 遮蔽像素设置为0
    return masked_bpps, mask
...
def save_weights(model,optimizer,epoch,folder):
    ...
    return folder+'/epoch{}.ckpt'.format(epoch+1)
